...
const Hero = () => {
    const [currentIndex, setCurrentIndex]  = useState(1);   //当前视频id？
    const [hasClicked, setHasClicked] = useState(false);    //有没有点击？
    const [isLoading, setIsLoading] = useState(true);     //isLoading
    const [loadedVideos, setLoadedVideos] = useState(0);   //加载第几个视频？
    const totalVideo = 4;    //总共三个视频
    const nextVideoRef = useRef(null)    //下一个视频的Ref
    const handleVideoLoad = () =>{
        setLoadedVideos((prev)=>prev + 1)    //处理加载视频
    }
